  ...
  return (
    <div className="job-wrapper w-full mx-auto md:w-10/12 text-md p-5">
      <div className="job-template  p-5 shadow-lg relative bg-white rounded-md m-5 md:flex justify-between">
        <img
          className="w-[40px] absolute -top-4 block md:hidden"
          src={logo}
          alt=""
        />
        ...
        <div className="lower-template flex-shrink md:inline my-3 md:my-0">
          <p
            className="role font-medium inline p-1 mx-2 rounded-sm cursor-pointer text-color4 bg-color3"
            onClick={() => {
              setJobs(jobs.filter((job) => job.role === role));
              handleFilterAdd({ name: "role", value: role });
            }}
            onDoubleClick={() => handleFilterDel(role)}
          >
            {role}
          </p>
          <p
            className="level font-medium inline p-1 mx-2 rounded-sm cursor-pointer text-color4 bg-color3"
            onClick={() => {
              setJobs(jobs.filter((job) => job.level === level));
              handleFilterAdd({ name: "level", value: level });
            }}
            onDoubleClick={() => handleFilterDel(level)}
          >
            {level}
          </p>
          <div className="languages md:inline my-3 md:my-0">
            {languages.map((language, index) => (
              <p
                ...
                onClick={() => {
                  setJobs(
                    jobs.filter((job) => job.languages.includes(language))
                  );
                  handleFilterAdd({ name: "languages", value: language });
                }}
                onDoubleClick={() => handleFilterDel(language)}
              >
                {language}
              </p>
            ))}
          ...
          <div className="tools md:inline my-3 md:my-0">
            {tools.map((tool, index) => (
              <p
                ...
                onClick={() => {
                  setJobs(
                    jobs.filter((job) => job.tools.includes(tool))
                  );
                  handleFilterAdd({ name: "tools", value: tool }) }}
                onDoubleClick={() => handleFilterDel(tool)}
              >
                {tool}
              </p>
            ))}
